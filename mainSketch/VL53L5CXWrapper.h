/**
 * @file VL53L5CXWrapper.h
 * @brief Wrapper class for managing multiple VL53L5CX Time-of-Flight multi-zone
 * distance sensors.
 *
 * This module provides a convenient interface for initializing, controlling,
 * and reading from multiple VL53L5CX sensors connected through a TCA9548A I2C
 * multiplexer. The VL53L5CX is a multizone Time-of-Flight ranging sensor that
 * provides distance measurements in an 8x8 or 4x4 grid format.
 */
 ...
 * @brief Wrapper class for managing multiple VL53L5CX multizone Time-of-Flight
 * sensors.
 *
 * This class provides a unified interface for controlling multiple VL53L5CX
 * sensors connected through a TCA9548A I2C multiplexer. It handles sensor
 * initialization, configuration, and reading distance data from all sensors in
 * an 8x8 grid format. Each sensor provides distance measurements for 64 zones
...
public:
   ...
   */
  VL53L5CXWrapper(const int addresses[], int addrCount, const int *tcaChans,
                  TCA9548A &tcaRef)
  ...
  /**
   * @brief Collects data from all initialized sensors and stores it in the
   * provided array.
          ...
          if (sensors[i].bGetRaningData()) {
            for (int y = 0; y < 8; y++) {
              for (int x = 0; x < 8; x++) {
                ...
                readings[i][y][x] =
                    (distance < 0) ? 0
                                   : distance; // Replace invalid data with 0
              }
            }
          } else {
